...
function scrapeMotionValuesFromProps(props, prevProps, visualElement) {
  ...
  for (const key in style2) {
    if (isMotionValue2(style2[key]) || prevProps.style && isMotionValue2(prevProps.style[key]) || isForcedMotionValue(key, props) || visualElement?.getValue(key)?.liveStyle !== void 0) {
...
function notifyTarget2({ target, contentRect, borderBoxSize }) {
  resizeHandlers2.get(target)?.forEach((handler) => {
    handler({
      target,
